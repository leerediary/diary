// Two-device convergence + RLS-scoping binary harness — Phase 7 / Plan 07-02.
//
// Proves SYNC-05: two of the user's devices, after editing offline, converge to
// byte-identical state through the Phase-6 sync engine (deletes propagate, nothing
// resurrects). Proves SYNC-06 scoping: owner JWT can read/write (positive) AND
// no-JWT/anon is rejected (negative).
//
// Mirrors test/sync-engine-verify.ts structure exactly: same check()/report[]/failures
// harness, same D-04 local-only host guard extended with owner-cred guard, same
// aligned-table report format. Also mirrors test/lww-verify.ts for createBrowserClient()
// anon usage (D-11 negative assertion).
//
// Two-device model (T-7-07): DiaryDB constructor is parameterless (super('diary-web')).
// Two simultaneous independent stores are NOT achievable via new DiaryDB('name').
// Device A and Device B are simulated as alternating snapshots of the single
// module-scope db: between device "turns" the local store + outbox + sync_meta are
// cleared and the device's view is reconstructed strictly by pulling from the shared
// LOCAL Supabase. Convergence is judged by server-returned updated_at on both
// reconstructed views — the same proven technique sync-engine-verify.ts uses.
//
...
async function main() {
  ...
  // (D-10 scenario 1 — T-7-08 server-clock baseline)
  {
    ...
    const localA = new LocalRepository(db)
    ...
    const localB = new LocalRepository(db)
  ...
  // (D-10 scenario 2 — T-7-04 core; both directions are distinct decideSyncAction paths)
  {
    ...
    // ── C2 Direction 1: A soft-deletes, B (stale offline) edits → tombstone wins ──
    {
      ...
      const localSeed = new LocalRepository(db)
  ...
  // then both went offline and edited the same date's journal.
  {
    ...
    const seedLocal = new LocalRepository(db)
  ...
  // (D-10 scenario 4)
  {
    ...
    const localC4A = new LocalRepository(db)
    ...
    await clearLocal()
    const localC4B = new LocalRepository(db)
  ...
  // The two-state (owner vs no-JWT) proof is sufficient and is asserted here.
  {
    ...
    const localD11 = new LocalRepository(db)
